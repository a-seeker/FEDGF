...
if __name__ == '__main__':
    ...
    for i in range(class_num):
        if args.dataset == 'cifar' or args.dataset == 'cinic':
            netEncoder = AutoEncoderCifar10(base_channel_size=32, latent_dim=64).cuda()
        else:
    ...
    for epoch in range(args.epochs):
        ...
        print("-----local ae start training-----")
        for client_id in idxs_users_ae:
            local = Client(args=args, dataset=dataset_train, idxs=dict_users[client_id], client_id=client_id, global_round= epoch)
            coder, coder_losses, enc = local.trainCoder2(coder_list[ae_epoch])
            if coder != None:
                coder_temp_loss[ae_epoch].append(coder_losses)
                new_enc_list.append(copy.deepcopy(enc))
                weights2.append(len(local.dataloader2))
        ...
        # ----- 编码器的特征聚合-----
        # b = sum(weights2)
        # newEnc = new_enc_list[0] * (weights2[0] / b)
        # for i in range(1, len(new_enc_list)):
        #     newEnc = newEnc + new_enc_list[i] * (weights2[i] / b)
